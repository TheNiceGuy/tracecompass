...
public abstract class SwtXYChartViewer extends TmfViewer implements IChartViewer {
     ...
     */
    private final Map<@NonNull ChartSeries, @NonNull ISeries> fSeriesMap = new HashMap<>();
    /**
    ...
    public void populate() {
        ...
        /* Configure axes */
        configureAxes();
        Stream.of(getChart().getAxisSet().getYAxes()).forEach(a -> a.enableLogScale(getModel().isXLogscale()));
        Stream.of(getChart().getAxisSet().getYAxes()).forEach(a -> a.enableLogScale(getModel().isYLogscale()));
     ...
     */
    protected abstract void createSeries(Map<@NonNull ChartSeries, @NonNull ISeries> mapper);
     ...
     */
    protected abstract void configureSeries(Map<@NonNull ISeries, Object[]> mapper);
    ...
    protected void validateChartData() {
        ...
        if (!getXDescriptorsInfo().areNumericals() && getModel().isXLogscale()) {
        ...
        if (!getYDescriptorsInfo().areNumericals() && getModel().isYLogscale()) {
            throw new IllegalArgumentException("Cannot have logarithmic scale on discontinuous data."); //$NON-NLS-1$
        }
    }
    ...
    protected String generateXTitle() {
        ...
        if (getData().getChartSeries().size() == 1) {
             ...
             */
            long nbDiffDescriptorName = getXDescriptors()
                    .map(descriptor -> descriptor.getName())
                    .distinct()
                    .count();

            long nbDiffDescriptorUnits = getXDescriptors()
    ...
    protected String generateYTitle() {
        ...
        if (getData().getChartSeries().size() == 1) {
             ...
             * same units, display that.
             */
            long nbDiffDescriptorName = getYDescriptors()
                    .map(descriptor -> descriptor.getName())
                    .distinct()
                    .count();

            long nbDiffDescriptorUnits = getYDescriptors()
                    .map(descriptor -> descriptor.getUnit())
                    .distinct()
                    .count();
    ...
    /**
     * Util method to clamp the external range of a range map. It returns the
     * map passed in parameter for chaining the method.
     *
     * @param map
     *            The map to clamp the external range
     * @return The same map passed in parameter
     */
    protected static ChartRangeMap clampExternalRange(ChartRangeMap map) {
        map.getExternal().clamp();
    ...
    protected static double getLogarithmicEspilon(ChartRangeMap map) {
        BigDecimal minBig = map.getExternal().getMinimum();
        BigDecimal maxBig = map.getExternal().getMaximum();
        ...
        /* Calculate epsilon */
        double delta = map.getExternal().getDelta().doubleValue();
    ...
    protected static Format getContinuousAxisFormatter(ChartRangeMap map, DescriptorsInformation info) {
        ...
        if (info.areTimestamps()) {
            BigDecimal max = map.getExternal().getMinimum();
            BigDecimal min = map.getExternal().getMaximum();
     ...
     */
    protected Map<@NonNull ChartSeries, @NonNull ISeries> getSeriesMap() {
        return fSeriesMap;
    }
    ...
    /**
     * Accessor that returns a stream of the X descriptors.
     *
     * @return The stream of X descriptors
     */
    protected Stream<IDataChartDescriptor<?, ?>> getXDescriptors() {
        return checkNotNull(getData().getChartSeries().stream().map(series -> series.getX()));
    }

    /**
     * Accessor that returns a stream of the Y descriptors.
     *
     * @return The stream of Y descriptors
     */
    protected Stream<IDataChartDescriptor<?, ?>> getYDescriptors() {
        return checkNotNull(getData().getChartSeries().stream().map(series -> series.getY()));
    }
    ...
    protected class LogarithmicPredicate implements Predicate<@Nullable Number> {
         ...
         */
        protected LogarithmicPredicate(IDataResolver<?, ?> resolver) {
            fResolver = checkNotNull(INumericalResolver.class.cast(resolver));
        }
