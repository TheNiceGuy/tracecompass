...
public abstract class XYChartViewer implements IChartViewer {
     ...
     */
    protected ChartRange fXExternalRange = new ChartRange(checkNotNull(BigDecimal.ZERO), checkNotNull(BigDecimal.ONE));
    /**
     * External Y range of the data
     */
    protected ChartRange fYExternalRange = new ChartRange(checkNotNull(BigDecimal.ZERO), checkNotNull(BigDecimal.ONE));
    ...
    protected void assertData() {
        ...
        @Nullable
        Boolean xContinuous = isXContinuous();
        if (xContinuous == null) {
        ...
        @Nullable
        Boolean yContinuous = isYContinuous();
        if (yContinuous == null) {
            throw new IllegalArgumentException("Each aspect must have the same continuity."); //$NON-NLS-1$
        }
    ...
    /**
     * This method should be called after the constructor. The factory
     * constructor handles that.
     */
    ...
    /**
     * This method is used to create series. It is called by the
    ...
    /**
     * This method post processes the data generated by
     * {@link #generateXData(List)} and {@link #generateYData(List)} in order to
     * affect it to the serie. For example, it should reject wrong values before
     * putting it in the serie. It is called by the {@link #populate()} method.
     */
    ...
    /**
     * This method refreshed the display labels. It is called by the
     * {@link #populate()} method.
     */
    ...
    protected String generateXTitle() {
        ...
        if (fData.getXData().size() == 1) {
             ...
             * X axis.
             */
            title = fData.getX(0).getAspect().getLabel();
        } else {
            /*
            ...
            if (nbDiffAspectName == 1) {
                title = fData.getX(0).getAspect().getName();
            }

            String units = null;
            if (nbDiffAspectsUnits == 1) {
                units = fData.getX(0).getAspect().getUnits();
            }
    ...
    protected String generateYTitle() {
        ...
        if (fData.getYData().size() == 1) {
             ...
             */
            title = fData.getY(0).getAspect().getLabel();
            ...
            title = "Value"; //$NON-NLS-1$
            if (nbDiffAspectName == 1) {
                title = fData.getY(0).getAspect().getName();
            }

            String units = null;
            if (nbDiffAspectsUnits == 1) {
                units = fData.getY(0).getAspect().getUnits();
            }
     ...
     * @return {@code true} is all aspects are continuous, {@code false} if all
     *         aspects are discontinuous or {@code null} if it shares both
     *         continuity
     */
    protected static @Nullable Boolean areAspectsContinuous(List<DataDescriptor> descriptors) {
        long count = descriptors.stream()
                .map(descriptor -> descriptor.getAspect() instanceof IDataChartNumericalAspect)
                .distinct()
                .count();

        if (count == 1) {
            return checkNotNull(descriptors.get(0)).getAspect() instanceof IDataChartNumericalAspect;
        }
     ...
     * into the aspects too, rather than using string.
     *
     * @param base
     ...
     */
    protected static void generateLabelMap(Stream<@Nullable String> stream, Map<@Nullable String, @NonNull Integer> map) {
    ...
    protected static ChartRange getRange(List<DataDescriptor> descriptors, boolean clampToZero) {
        ...
        for (DataDescriptor descriptor : descriptors) {
            ...
            num = source.getStream()
                    .filter(Objects::nonNull)
                    .min(source.getComparator())
     ...
     *
     * The internal value = ((rawValue - rawMinimum) * (internalRangeDelta /
     * rawRangeDelta)) + internalMinimum
     *
     * @param number
    ...
    protected static Format getContinuousAxisFormatter(List<DataDescriptor> axisAspects, @Nullable ChartRange internalRange, @Nullable ChartRange externalRange) {
        ...
        if (areAspectsTimeStamp(axisAspects)) {
            /*
             * Set a TimeStamp formatter depending on the duration between the
            ...
            for (DataDescriptor descriptor : axisAspects) {
                ...
                value = checkNotNull(source.getStream()
                        .filter(Objects::nonNull)
                        .min(source.getComparator())
                        .orElse(Long.MAX_VALUE))
                                .toString();
            ...
            /*
             * For other numeric aspects, use the default decimal unit
             * formatter.
             */
     ...
     * @return {@code true} is all aspects are time stamps, otherwise
     *         {@code false}
     */
    protected static boolean areAspectsTimeStamp(List<DataDescriptor> descriptors) {
        return descriptors.stream().allMatch(aspect -> aspect.getAspect() instanceof GenericDataChartTimestampAspect);
    }
     ...
     * @param descriptors
     *            The list of aspects to check.
     * @return {@code true} is all aspects are time durations, otherwise
     *         {@code false}
     */
    protected static boolean areAspectsTimeDuration(List<DataDescriptor> descriptors) {
        return descriptors.stream().allMatch(aspect -> aspect.getAspect() instanceof GenericDataChartDurationAspect);
    }
    ...
    /**
     * This accessor return whether the X axis is continuous or not. If null is
     * returned, then the axis is both continuous and discontinuous.
     *
    ...
    /**
     * This accessor return whether the Y axis is continuous or not. If null is
     * returned, then the axis is both continuous and discontinuous.
     *
    ...
    private class MouseEnterEvent implements Listener {
        ...
        public void handleEvent(@Nullable Event event) {
            ...
            /*
             * Only set to visible if we are at the right location, in the right
             * shell.
             */
